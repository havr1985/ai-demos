...
const UNAVAILABLE_ERROR =
  "The shop assistant is temporarily unavailable. Please try again in a moment.";
...
export async function POST(req: Request) {
  ...
  const webhookUrl = process.env.N8N_SHOP_WEBHOOK_URL;
  if (!webhookUrl) {
    console.error("[shop-chat] N8N_SHOP_WEBHOOK_URL is not set");
    return errorResponse(500, GENERIC_ERROR);
  }

  console.log("[shop-chat] →", {
  ...
  try {
    ...
    console.error("[shop-chat] n8n unreachable", { name, message: errMessage });
  ...
  if (!upstream.ok) {
    ...
    console.error("[shop-chat] n8n non-2xx", {
  ...
  try {
    ...
    console.error("[shop-chat] n8n returned non-JSON", { message: errMessage });
    return errorResponse(500, GENERIC_ERROR);
  }

  console.log("[shop-chat] ← n8n ok", {
    sessionId: shortSid(sessionId),
    ms: elapsedMs,
  });
